...
ifeq ($(DEBUG_MODE),1)
        C_OPS+=-DDEBUG -DAVISOR_GUEST_PATCH_DEBUG
endif
ASM_OPS = -I$(INC_DIR)
...
qemu.img: $(LINKER_FILE_QEMU) $(OBJ_FILES)
	...
	mkdir -p ./bin
	cp $(BUILD_DIR)/kernel8.img $(OUTDIR)/kernel8.img
	# Some toolchains' aarch64-objdump may abort on certain system-register encodings.
	# This disassembly is optional, so don't fail the build if objdump crashes.
	$(GNU_OBJDUMP) -D $(BUILD_DIR)/kernel8.elf > $(BUILD_DIR)/kernel8.list || true
