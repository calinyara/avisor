...
static uint64_t emulate_mbox_read(struct avisor_vcpu *vcpu)
{
	/*
	...
	if ((mbox_val & 0xF) == MBOX_CHAN_TAGS && mbox[1] == MBOX_REQUEST) {
		...
		case MBOX_TAG_SET_POWER_STATE:
			...
			break;
		default:
			WARN("Unsupported mbox TAG:%x", mbox[2]);
		}
	}
...
static uint64_t emulate_mbox_write(struct avisor_vcpu *vcpu, uint64_t val)
{
	 ...
	 */
	if (!check_vcpu_page_mapped(vcpu->vm, gva)) {
		PANIC("mbox buf not mapped!!!\n");
		return -EFAULT;
	}

	gvirt_to_maddr(gva, &maddr, GV2M_WRITE);
	mbox = (uint32_t *)maddr;
...
uint64_t handle_mbox_read(struct avisor_vcpu *vcpu, uint64_t addr)
{
	uint64_t ret = 0;
...
uint64_t handle_mbox_write(struct avisor_vcpu *vcpu, uint64_t addr, uint64_t val)
{
	uint64_t ret = 0;
	switch (addr) {
	case MBOX_WRITE:
		ret = emulate_mbox_write(vcpu, val);
		break;
	default:
