...
static int32_t shell_cmd_test(int32_t argc, char **argv);
...
static struct shell_cmd shell_cmds[] = {
	...
	{
		.str = SHELL_CMD_TEST,
		.cmd_param = SHELL_CMD_TEST_PARAM,
		.help_str = SHELL_CMD_TEST_HELP,
		.fcn = shell_cmd_test,
	},
};
...
static int32_t shell_cmd_vmc(int32_t argc, char **argv)
{
	...
	snprintf(temp_str, MAX_OUTPUT_LEN, "--> Switched to VM %d <--\r\n", vm_id);
	shell_puts(temp_str);
...
static int32_t shell_cmd_vmld(int32_t argc, char **argv)
{
	...
	(void)strncpy(config_args.filename, argv[1], 36);
	config_args.load_addr = load_addr;
	config_args.entry_point = entry_addr;
	config_args.core_id = core_id;

	vm_id = get_nr_vm_created();
	if (create_vm(vm_id, &config_args) < 0) {
...
static int32_t shell_cmd_test(int32_t argc, char **argv)
{
	printf("tick0=%llu, tick1=%llu, tick2=%llu, tick3=%llu \n",
		core0_tick_count, core1_tick_count, core2_tick_count, core3_tick_count);
	return 0;
}
