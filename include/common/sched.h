...
#define THREAD_CPU_CONTEXT 0 // offset of cpu_context in avisor_vcpu

#ifndef __ASSEMBLER__
#define THREAD_SIZE 4096

#define MAX_VCPUS (MAX_VCPU_PER_CORE)

#define VCPU_RUNNING 0
#define VCPU_ZOMBIE  1
#define VCPU_STOPPED 2
...
struct avisor_vcpu {
	...
	long priority;
	long preempt_count;
	long pid;  	// vcpu id on this pcpu
	long vmid; 	// used as VMID
	long pcpu_id;	// pcpu id
	long vcpu_id;	// vcpu_id in VM, now always 0
	unsigned long flags;
...
extern void set_cpu_virtual_interrupt(struct avisor_vcpu *vcpu);
extern int is_cntv_irq_pending(void);
void set_cpu_sysregs(struct avisor_vcpu *vcpu);
extern void switch_to(struct avisor_vcpu *next);
extern void cpu_switch_to(struct avisor_vcpu *prev, struct avisor_vcpu *next);
extern void stop_vcpu(void);
extern void show_vcpu_list(void);
